...
const CONSTANTS = {
    ...
    FILES: {
        DATABASE: 'scriptum-data.json',
        BACKUP_PREFIX: 'scriptum-backup-'
    },
...
window.onload = async function() {
    ...
    const fileLoaded = await loadDatabaseFromFile();
    if (!fileLoaded) {
        loadData();
        loadReadingListData();
        loadMyLibraryData();
...
function showView(viewName, buttonElement) {
    ...
    if (viewName === CONSTANTS.VIEWS.DASHBOARD) {
        ...
        renderMyLibrary();
    } else if (viewName === CONSTANTS.VIEWS.ADMIN) {
        // Admin view needs no special initialization
    }
}
...
async function selectBackupFolder() {
    try {
        ...
        if (result.success) {
            document.getElementById('backupFolder').value = result.path;
        }
    } catch (error) {
        showMessage('Error selecting folder', CONSTANTS.MESSAGE_TYPES.ERROR);
    }
}
...
function loadSettings() {
    setTimeout(() => {
        ...
        if (pagesInput) pagesInput.value = dailyReadingPages;
        if (folderInput) folderInput.value = backupFolder;
    }, 50);
}
...
async function saveSettings(event) {
    ...
    const backupFolder = document.getElementById('backupFolder').value;
    ...
    // Save backup folder
    if (backupFolder) {
        localStorage.setItem(CONSTANTS.STORAGE_KEYS.BACKUP_FOLDER, backupFolder);
    } else {
        localStorage.removeItem(CONSTANTS.STORAGE_KEYS.BACKUP_FOLDER);
    }
    
    // Save to unified database file to include settings
