...
// Save Database - saves uncompressed JSON to Downloads
function saveDatabaseFile() {
    ...
    showMessage('Database saved as scriptum-data.json', CONSTANTS.MESSAGE_TYPES.SUCCESS);
}


// Backup Database - saves as bt-data-YYYYMMDD.json.gz (compressed if available)
async function backupDatabaseFile() {
    const backupFolder = localStorage.getItem(CONSTANTS.STORAGE_KEYS.BACKUP_FOLDER);
    
    if (!backupFolder) {
        showMessage('Please select a backup folder in Settings first', CONSTANTS.MESSAGE_TYPES.ERROR);
        return;
    }
    
    try {
        // Validate folder
        const validation = await window.electronAPI.validateBackupFolder(backupFolder);
        if (!validation.success) {
            showMessage(`Backup folder error: ${validation.error}`, CONSTANTS.MESSAGE_TYPES.ERROR);
            return;
        }
        
        if (validation.created) {
            showMessage('Backup folder created successfully', CONSTANTS.MESSAGE_TYPES.INFO);
        }
        
        // Create and save backup
        const dataToExport = generateUnifiedDatabase();
        const result = await window.electronAPI.saveCompressedBackup(dataToExport, backupFolder);
        
        if (result.success) {
            showMessage(`Database backup saved as ${result.filename}`, CONSTANTS.MESSAGE_TYPES.SUCCESS);
        } else {
            throw new Error(result.error);
        }
    } catch (error) {
        showMessage(`Backup failed: ${error.message}`, CONSTANTS.MESSAGE_TYPES.ERROR);
    }
}
