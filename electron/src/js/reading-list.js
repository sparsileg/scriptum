...
async function startFinishingBook(readingListId) {
    ...
    if (readingListBook.MyLibraryId) {
        ...
        // For "Other" source books, try ISBN lookup with timeout
        showMessage('Looking up book information...', CONSTANTS.MESSAGE_TYPES.INFO);
        
        try {
            // Add timeout to ISBN lookup
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Lookup timeout')), 10000) // 10 second timeout
            );
            
            const lookupPromise = lookupISBN(bookData.title, bookData.author);
            const isbnData = await Promise.race([lookupPromise, timeoutPromise]);
            ...
            if (isbnData && isbnData.confidence > CONSTANTS.CONFIDENCE_LEVELS.GOOD_MATCH) {
                ...
                showMessage('Book information found', CONSTANTS.MESSAGE_TYPES.SUCCESS);
            } else {
                showMessage('No ISBN information found, continuing...', CONSTANTS.MESSAGE_TYPES.INFO);
            }
        } catch (error) {
            console.error('ISBN lookup failed:', error);
            showMessage('ISBN lookup failed or timed out, continuing...', CONSTANTS.MESSAGE_TYPES.INFO);
