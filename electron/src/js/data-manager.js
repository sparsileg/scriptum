...
function generateUnifiedDatabase() {
    ...
    return {
        ...
        "Settings": {
            ...
            "backupFolder": localStorage.getItem(CONSTANTS.STORAGE_KEYS.BACKUP_FOLDER) || null,
...
// Add these new functions at the end of data-manager.js

// File-based save function
async function saveDatabaseToFile() {
    try {
        const unifiedData = generateUnifiedDatabase();
        const result = await window.electronAPI.saveDatabase(unifiedData);
        
        if (result.success) {
            console.log(`Database saved to: ${result.path}`);
            return true;
        } else {
            throw new Error(result.error);
        }
    } catch (error) {
        console.error('File save failed, falling back to localStorage:', error);
        
        // Fallback to localStorage
        DataManager.saveSection('BooksRead', books);
        DataManager.saveSection('MyLibrary', myLibrary);
        DataManager.saveSection('ReadingList', readingList);
        
        showMessage(`Database save failed: ${error.message}. Using local backup.`, CONSTANTS.MESSAGE_TYPES.ERROR);
        return false;
    }
}

// File-based load function
async function loadDatabaseFromFile() {
    try {
        ...
        if (result.success) {
            ...
            if (data.Settings) {
                if (data.Settings.displayTheme) {
                    changeTheme(data.Settings.displayTheme);
                }
                if (data.Settings.dailyReadingPages) {
                    localStorage.setItem(CONSTANTS.STORAGE_KEYS.DAILY_READING_PAGES, 
                                       data.Settings.dailyReadingPages.toString());
                }
                if (data.Settings.dashboardOrder) {
                    DataManager.saveSection('DashboardOrder', data.Settings.dashboardOrder);
                }
            }
            ...
            console.log('No existing database found - new user');
            return false;
        } else {
            throw new Error(result.error);
        }
    } catch (error) {
        console.error('File load failed, falling back to localStorage:', error);
        ...
        showMessage(`Database load failed: ${error.message}. Using local backup.`, CONSTANTS.MESSAGE_TYPES.ERROR);
        return false;
    }
}

// Replace existing save functions
function saveData() {
    return saveDatabaseToFile();
}
